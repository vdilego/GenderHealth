...
# after taking out the non-eligible individuals we have 15,464 observations.


# Reading HRS for all years


mhas <- read_dta(here("Gender_health","Data","MHAS","Stata_Data","mhas_02272023.dta")) # read-in stata file from merge procedure
...
df3<-df2 %>% 
  filter(!is.na(rwtresp) & wave==4 )    %>% 
  filter(rwtresp>0)  # filtering only the eligible cases: non-eligibiles have a missing id               # filtering only the eligible cases: non-eligibiles have a missing id
...
plot_missy<-df3 %>%
  ...
  select(c("rdiabe_1", "rcancre_1","rlunge_m_1","rhearte_1","rstroke_1","rarthre_1","radlfive","ragey")) %>%
  # Create an UpSet plot
  gg_miss_upset(., nsets = 10 )


pdf(here("Gender_health", "Countries","MEXICO","Descriptive","missing_mhas.pdf"))
plot_missy
dev.off()


# check all variables  by gender:
pdf(here("Gender_health", "Countries","MEXICO","Descriptive","missing_mhas_gender.pdf"))
gg_miss_var(df3, ragender_2, show_pct = T)
dev.off()

# missing patterns is not different by sex, but is not trivial. We have to analyse it first.
...
df3_na<-df3 %>% 
  select(-c(raeducl_1, raeducl_2)) %>% 
  drop_na()

df3_var<-df3_na %>% 
  group_by(unhhidnp) %>% 
  mutate(chronic_free = case_when(rdiabe_1 ==0 & rcancre_1 == 0 &
                                    rlunge_m_1==0   & rhearte_1 ==  0 &
         ...
         chronic_free_lunge = case_when(rlunge_m_1 ==0 ~ 0, TRUE ~ 1)) %>% 
  mutate(chronic_sum=sum(rdiabe_1,rcancre_1,rlunge_m_1,rhearte_1,rstroke_1,rarthre_1)) %>% 
  ungroup() %>% 
  group_by(unhhidnp) %>% 
...
prop.table(svytable(~ragender_2, design=df_mhas_surv))    #weighted
prop.table(svytable(~age_cat, design=df_mhas_surv))

prop.table(svytable(~adl, design=df_mhas_surv))

# Now accounting for more than one variable
svytable(~radlfive+ragey, design=df_mhas_surv)

svytable(~radlfive+age_cat+ragender_2, design=df_mhas_surv)

svytable(~chronic_free+age_cat+ragender_2, design=df_mhas_surv)

svytable(~chronic_sum+age_cat+ragender_2, design=df_mhas_surv)

svytable(~chronic_severe+age_cat+ragender_2, design=df_mhas_surv)

# table with summary statistics

svyranktest(chronic_free~ragender_2, df_mhas_surv, test="median")

df_mhas_surv %>% 
  tbl_svysummary(
    # Use include to select variables
    by=ragender_2,
    include = c(ragender_2,adl,ragey, chronic_free,chronic_free_heart,chronic_free_diabe,
...
as_flex_table() %>%
  flextable::save_as_docx(path=here("Gender_health","Countries","MEXICO","Descriptive","decriptive_mexico.docx"))
...
write.table(prop_healthy_adl, here("Gender_health","Data","MHAS","Prevalence", "prev_mhas_adl_agecat.csv"),
            sep = ",", row.names = F)
...
write.table(prop_healthy_chron, here("Gender_health","Data","MHAS","Prevalence", "prev_mhas_cron_agecat.csv"),
            sep = ",", row.names = F)
...
pdf(here("Gender_health", "Countries","MEXICO","Descriptive","fig_prev_mexico.pdf"), width = 10, height=10)
fig_prev_all
dev.off()
...
pdf(here("Gender_health", "Countries","MEXICO","Descriptive","fig_prevProp_mexico.pdf"), width = 13, height=6)
fig_prevProp
dev.off()
...
write.table(prop_all_chronic, here("Gender_health","Data","MHAS","Prevalence",
                                   "prev_mhas_cron_type_agecat.csv"), sep = ",", row.names = F)


# some models
library(ggeffects)

mod1<-svyglm(as.numeric(adl)~age_cat+ragender_2, design= df_mhas_surv, family = binomial)
summary(mod1)
...
#mod2<-svyglm(as.numeric(adl)~age_cat+ragender_2+raeducl_2, design= df_mhas_surv, family = binomial)
...
mod3<-svyglm(as.numeric(chronic_free)~age_cat+ragender_2, design= df_mhas_surv, family = binomial)
...
mod4<-svyglm(as.numeric(chronic_free_heart)~age_cat+ragender_2, design= df_mhas_surv, family = binomial)
summary(mod4)
...
#mod5<-svyglm(as.numeric(chronic_free_heart)~age_cat+ragender_2+raeducl_2, design= df_mhas_surv, family = binomial)
#summary(mod5)
...
mod6<-svyglm(as.numeric(chronic_free_arthre)~age_cat+ragender_2, design= df_mhas_surv, family = binomial)
summary(mod6)
...
mod7<-svyglm(as.numeric(chronic_sum)~age_cat+ragender_2, design= df_mhas_surv, family=poisson)
summary(mod7)
