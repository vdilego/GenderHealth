...
# WE use Version G.2 incorporates the latest released version of ELSA data, which includes eleven main modules and 
...
# After deleting all those cases, we have a sample size of 8152 observations. After excluding 2 missing from adl 


elsa <- read_dta(here("Gender_health","Data","ELSA","Stata_Data","elsa_02272023.dta")) # read-in stata file from merge procedure

df1<-as_factor(elsa)                         # transform all using as_factor from haven    
df1<-df1 %>% 
  mutate(year = case_when(wave ==7 ~ '2015',
                          TRUE ~ 'NA'))
...
df3<-df2 %>% 
  ...
  filter(rcwtresp>0)  # filtering only the eligible cases: non-eligibiles have a missing id
...
# when thinking in terms of missing values. # no missing for ELSA because all already excluded

# basically no missing
df3 %>%
  # Select the survey items
  select(c("rdiabe_1", "rcancre_1","rlunge_1","rhearte_1","rstroke_1","rarthre_1","radlfive","raeducl_2")) %>%
  # Create an UpSet plot
  gg_miss_upset(., nsets = 10 )


gg_miss_var(df3, radlfive, show_pct = T)
gg_miss_var(df3, radlfive, show_pct = T)
...
# here we only exclude the adl missing for the analysis, not the varible education as that is not very 
# relevant for the health estimates per se.

df3_na<-df3 %>% 
  filter(!is.na(radlfive))
...
# converting to survey data to account for weights. ELSA provides individual-level or household weights 
# that account for non-response adjustment (rcwtresp), and no weights to account for complex
# survey design .
...
df_elsa_surv <- svydesign( 
  ...
  strata= ~rstrat,
  nest = T, 
  data = subset(df3_var, rcwtresp > 0 & ragey>=50))            
...
prop.table(svytable(~ragender_2, design=df_elsa_surv))    #weighted
prop.table(svytable(~age_cat, design=df_elsa_surv))

prop.table(svytable(~adl, design=df_elsa_surv))

# Now accounting for more than one variable
svytable(~radlfive+ragey, design=df_elsa_surv)

svytable(~radlfive+age_cat+ragender_2, design=df_elsa_surv)

svytable(~chronic_free+age_cat+ragender_2, design=df_elsa_surv)

svytable(~chronic_sum+age_cat+ragender_2, design=df_elsa_surv)

svytable(~chronic_severe+age_cat+ragender_2, design=df_elsa_surv)

# table with summary statistics

tbl <- svytable(~chronic_free+ragender_2, design=df_elsa_surv)

summary(tbl, statistic="Chisq")

svychisq(~chronic_free+ragender_2, design=df_elsa_surv, statistic="adjWald")
...
df_elsa_surv %>% 
  tbl_svysummary(
    ...
    by=ragender_2,
    include = c(ragender_2,adl,chronic_free,chronic_free_heart,chronic_free_diabe,
...
as_flex_table() %>%
  flextable::save_as_docx(path=here("Gender_health","Countries","ENGLAND","Descriptive","decriptive_elsa.docx"))
...
colnames(prop_healthy_adl)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                              "CI_up_unhealthy")
prop_healthy_adl<-prop_healthy_adl[, c(1:6,8,7,9)]
...
write.table(prop_healthy_adl, here("Gender_health","Data","ELSA","Prevalence", "prev_elsa_adl_agecat.csv"),
            sep = ",", row.names = F)
...
colnames(prop_healthy_chron)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                "CI_up_unhealthy")
prop_healthy_chron<-prop_healthy_chron[, c(1:6,8,7,9)]
...
write.table(prop_healthy_chron, here("Gender_health","Data","ELSA","Prevalence", "prev_elsa_cron_agecat.csv"),
            sep = ",", row.names = F)
...
colnames(prop_healthy_chron_heart)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                      "CI_up_unhealthy")
prop_healthy_chron_heart<-prop_healthy_chron_heart[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_arthre)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                       "CI_up_unhealthy")
prop_healthy_chron_arthre<-prop_healthy_chron_arthre[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_cancer)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                       "CI_up_unhealthy")
prop_healthy_chron_cancer<-prop_healthy_chron_cancer[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_stroke)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                       "CI_up_unhealthy")
prop_healthy_chron_stroke<-prop_healthy_chron_stroke[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_lunge)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                      "CI_up_unhealthy")
prop_healthy_chron_lunge<-prop_healthy_chron_lunge[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_diabe)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                      "CI_up_unhealthy")
prop_healthy_chron_diabe<-prop_healthy_chron_diabe[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_2)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                  "CI_up_unhealthy")
prop_healthy_chron_2<-prop_healthy_chron_2[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_3)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                  "CI_up_unhealthy")
prop_healthy_chron_3<-prop_healthy_chron_3[, c(1:6,8,7,9)]
...
colnames(prop_healthy_chron_4)<-c("age","year","gender","healthy","unhealthy","CI_low_healthy","CI_low_unhealthy","CI_up_healthy",
                                  "CI_up_unhealthy")
prop_healthy_chron_4<-prop_healthy_chron_4[, c(1:6,8,7,9)]
...
pdf(here("Gender_health", "Countries","ENGLAND","Descriptive","fig_prev_england.pdf"), width = 10, height=10)
fig_prev_all
dev.off()
...
pdf(here("Gender_health", "Countries","ENGLAND","Descriptive","fig_prevProp_england.pdf"), width = 13, height=6)
fig_prevProp
dev.off()
...
write.table(prop_all_chronic, here("Gender_health","Data","ELSA","Prevalence", "prev_elsa_cron_type_agecat.csv"),
            sep = ",",row.names = F)
...
# sample size is not enough to perform the analysis separately for age groups. 
mod1<-svyglm(as.numeric(adl)~ragender_2, design= df_elsa_surv,family = quasibinomial)
summary(mod1)


mod1.1<-svyglm(as.numeric(adl)~age_cat+ragender_2, design= df_elsa_surv,family = quasibinomial)
summary(mod1.1)

mod_1.1<-ggeffect(mod1.1, terms = c("age_cat", "ragender_2"))  %>%
  plot() 


mod3<-svyglm(as.numeric(chronic_free)~ragender_2, design= df_elsa_surv, family = binomial)
...
mod4<-svyglm(as.numeric(chronic_free_heart)~ragender_2, design= df_elsa_surv, family = quasibinomial)
summary(mod4)


mod5<-svyglm(as.numeric(chronic_free_heart)~ragender_2, design= df_elsa_surv, family = quasibinomial)
summary(mod5)


mod6<-svyglm(as.numeric(chronic_free_arthre)~ragender_2, design= df_elsa_surv, family = quasibinomial)
summary(mod6)


mod7<-svyglm(as.numeric(chronic_sum)~ragender_2, design= df_elsa_surv, family=poisson)
summary(mod7)
