...
# We will focus on Wave 5. However, because we focus on ages >50 and the eligible core sample, the sample is not 7,949, but 7029.
...
klosa <- read_dta(here("Gender_health","Data","KLOSA","Stata_Data","klosa_02242023.dta")) # read-in stata file from merge procedure

df1<-as_factor(klosa)                         # transform all using as_factor from haven    
df1<-df1 %>% 
  mutate(year = case_when(wave ==5 ~ '2015',
                          TRUE ~ 'NA'))
...
pdf(here("Gender_health", "Countries","KOREA","Descriptive","missing_klosa.pdf"))
plot_missy
dev.off()


# check all variables  by gender:
pdf(here("Gender_health", "Countries","KOREA","Descriptive","missing_klosa_gender.pdf"))
...
prop.table(svytable(~ragender_2, design=df_klosa_surv))    #weighted
prop.table(svytable(~age_cat, design=df_klosa_surv))

prop.table(svytable(~adl, design=df_klosa_surv))

# Now accounting for more than one variable
svytable(~radlfive+ragey, design=df_klosa_surv)

svytable(~radlfive+age_cat+ragender_2, design=df_klosa_surv)

svytable(~chronic_free+age_cat+ragender_2, design=df_klosa_surv)

svytable(~chronic_sum+age_cat+ragender_2, design=df_klosa_surv)

svytable(~chronic_severe+age_cat+ragender_2, design=df_klosa_surv)

# table with summary statistics

svyranktest(chronic_free~ragender_2, df_klosa_surv, test="median")
...
as_flex_table() %>%
  flextable::save_as_docx(path=here("Gender_health","Countries","KOREA","Descriptive","decriptive_klosa.docx"))
...
prop_healthy_adl<-svyby(formula = ~adl==1, by = ~age_cat+year+ragender_2, 
...
write.table(prop_healthy_adl, here("Gender_health","Data","KLOSA","Prevalence", "prev_klosa_adl_agecat.csv"), sep = ",", col.names = NA,
            qmethod = "double")
...
write.table(prop_healthy_chron, here("Gender_health","Data","KLOSA","Prevalence", "prev_klosa_cron_agecat.csv"), sep = ",", col.names = NA,
            qmethod = "double")
...
pdf(here("Gender_health", "Countries","KOREA","Descriptive","fig_prev_korea.pdf"), width = 10, height=10)
fig_prev_all
dev.off()
...
pdf(here("Gender_health", "Countries","KOREA","Descriptive","fig_prevProp_korea.pdf"), width = 13, height=6)
fig_prevProp
dev.off()
...
write.table(prop_all_chronic, here("Gender_health","Data","KLOSA","Prevalence", "prev_klosa_cron_type_agecat.csv"), sep = ",", col.names = NA,
            qmethod = "double")
...
mod1<-svyglm(as.numeric(adl)~age_cat+ragender_2, design= df_klosa_surv, family = binomial)
summary(mod1)
...
mod2<-svyglm(as.numeric(adl)~age_cat+ragender_2+raeducl_2, design= df_klosa_surv, family = binomial)
summary(mod2)
...
mod3<-svyglm(as.numeric(chronic_free)~age_cat+ragender_2, design= df_klosa_surv, family = binomial)
...
mod4<-svyglm(as.numeric(chronic_free_heart)~age_cat+ragender_2, design= df_klosa_surv, family = binomial)
summary(mod4)
...
mod5<-svyglm(as.numeric(chronic_free_heart)~age_cat+ragender_2+raeducl_2, design= df_klosa_surv, family = binomial)
summary(mod5)
...
mod6<-svyglm(as.numeric(chronic_free_arthre)~age_cat+ragender_2, design= df_klosa_surv, family = binomial)
summary(mod6)
...
mod7<-svyglm(as.numeric(chronic_sum)~age_cat+ragender_2, design= df_klosa_surv, family=poisson)
summary(mod7)
