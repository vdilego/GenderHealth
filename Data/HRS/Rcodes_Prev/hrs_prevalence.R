...
hrs <- read_dta(here("Gender_health","Data","HRS","Stata_Data","hrs_02222023.dta")) # read-in stata file from merge procedure
...
df2<-cSplit(df1, splitCols = c(3,9:15), ".")  # split the cols because of stata labels 

df3<-df2 %>% 
  filter(!is.na(rwtresp) )%>%                   # filtering only the eligible cases: non-eligibiles have a missing id
  select(- c(9))                               # select only the variables for use in prevalence estimation 
