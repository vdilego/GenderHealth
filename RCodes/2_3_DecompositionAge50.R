...
#slight change to function to see the DFLE at each age
...
for (i in 1:length(cntr)){
  ...
  #Outputs
  out_gap[[i]] <- data.frame (Age=c(50,55,60,65,70,75,80),
                             Country=cntr[i],
                             DFLE_f= HL_f,
                             DFLE_f.l= HL_f.l,
                             DFLE_f.u= HL_f.u,
...
en_mort_f<-fread(here(mort.folder,"lt_eng_abridged.csv")) %>%  
  ...
  filter(country%in%"England" & age>=50 & sex%in%"woman" )

en_mort_m<-fread(here(mort.folder,"lt_eng_abridged.csv")) %>%  
  #mutate(sex = fct_recode(sex, "man" = "Male")) %>% 
  mutate(country = "England") %>% 
  filter(country%in%"England" & age>=50 & sex%in%"man" )
